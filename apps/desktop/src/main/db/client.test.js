...
describe('SQLite Client', () => {
    ...
    it('should enable WAL mode (or memory for :memory: db)', () => {
        const result = client.db.pragma('journal_mode', { simple: true });
        // In-memory databases cannot use WAL mode, they default to 'memory'
        expect(['wal', 'memory']).toContain(result);
    });
    it('should enable foreign keys', () => {
        const result = client.db.pragma('foreign_keys', { simple: true });
    ...
    describe('CRUD operations', () => {
        ...
        it('should cascade delete from workspace to documents', () => {
            ...
            // Insert document
            client.db
                .prepare(`INSERT INTO documents (id, workspace_id, file_name, file_type, file_hash, title, raw_text, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        ...
        it('should insert and query llm_usage_records', () => {
            const now = new Date().toISOString();
            client.db
                .prepare('INSERT INTO llm_usage_records (id, request_type, model_id, input_tokens, output_tokens, cost, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
        ...
        it('should handle full annotation + discussion flow', () => {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            ...
            client.db
                .prepare(`INSERT INTO generated_articles (id, source_document_id, title, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?)`)
                .run('art-1', 'doc-1', 'Test Article', 'completed', now, now);
            client.db
                .prepare(`INSERT INTO generated_sections (id, article_id, "index", title, source_chapter_ids, content, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run('sec-1', 'art-1', 0, 'Section 1', '[]', 'Test content.', 'completed', now, now);
            // Create annotation
            client.db
                .prepare(`INSERT INTO annotations (id, article_id, section_id, anchor_start_offset, anchor_end_offset, anchor_exact_text, anchor_prefix, anchor_suffix, type, content, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run('ann-1', 'art-1', 'sec-1', 0, 4, 'Test', '', ' content', 'note', 'A note', now, now);
            // Add discussion messages
            client.db
                .prepare(`INSERT INTO discussion_messages (id, annotation_id, role, content, created_at)
           VALUES (?, ?, ?, ?, ?)`)
                .run('msg-1', 'ann-1', 'user', 'What is this?', now);
            client.db
                .prepare(`INSERT INTO discussion_messages (id, annotation_id, role, content, model_id, token_usage, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?)`)
